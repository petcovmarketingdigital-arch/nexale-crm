            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm shadow-indigo-900/5 border border-slate-100 p-6">
                ...
                <div className="mt-4 border-t border-slate-100 pt-4">
                  {(() => {
                    const filteredLeadsList = getCampFilteredLeadsList();
                    const selectedCrmCount = filteredLeadsList.filter(l => campSelectedLeads.includes(l.id)).length;
                    const activeCount = campTab === 'crm' ? selectedCrmCount : campTab === 'externa' ? externalContacts.length : (selectedCrmCount + externalContacts.length);
                    ...
                    return (
                      <>
                        <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
                          ...
                          >
                            {waCheckLoading ? '⏳ Checando WhatsApp...' : `🔍 Checar ${activeCount > 0 ? activeCount + ' ' : ''}Número(s) Ativo(s)`}
                          </button>
                        </div>
                        <p className="text-[11px] text-slate-400 font-medium">
                          {activeCount > 0 
                            ? `O sistema testará se os ${activeCount} contato(s) selecionado(s) possuem conta registrada no WhatsApp.`
              ...
              <form onSubmit={handleSaveLead} className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={formData.valor} onChange={e => setFormData({...formData, valor: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold text-indigo-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="0 (opcional)" />
                  </div>
                  <div>
